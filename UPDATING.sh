...
sed -i '' 's|- role: buluma\.|- role: buluma.roles.|' roles/*/README.md

# Use proper role FQCNs in role `dependencies:`
sed -i '' 's|- buluma\.|- buluma.roles.|' roles/*/meta/main.yml

# Regenerate all used collections.
echo "---" > requirements.yml
echo "collections:" >> requirements.yml
cat roles/*/requirements.yml | grep '  - name: ' | grep -v buluma | sort | uniq >> requirements.yml
